...
VERTEX_MODEL = os.getenv("VERTEX_GEMINI_MODEL", GEMINI_MODEL).strip() or GEMINI_MODEL
VERTEX_TIMEOUT_MS = int(os.getenv("VERTEX_TIMEOUT_MS", "12000"))
VERTEX_SERVICE_ACCOUNT_JSON = os.getenv("VERTEX_SERVICE_ACCOUNT_JSON", "").strip()
...
QUESTION_METADATA = {
    ...
    "q_04": {
        "title": "レシピの分量",
        "description": "4人分で小麦粉240g使う。同比率で6人分に必要な量を求める。",
        "solution_guide": (
            "1. 1人分を 240 / 4 = 60g としてから 60 * 6 = 360g とする。\n"
            "2. または 6人分は4人分の1.5倍と考え、240 * 1.5 = 360g とする。\n"
            "3. どちらの方針でも、比例関係を捉えた観察可能な途中式だけを根拠にする。"
        )
    },
    "q_05": {
        "title": "単価の比較",
        "description": "Aは6本780円、Bは10本1,250円。1本あたり安い方を求める。",
        "solution_guide": (
            "Aは 780 / 6 = 130円、Bは 1250 / 10 = 125円なのでBが1本あたり5円安い。"
        )
    },
    "q_06": {
        "title": "4日間の平均",
        "description": "20分、35分、25分、40分の1日あたり平均を求める。",
        "solution_guide": (
            "合計は 20 + 35 + 25 + 40 = 120分。日数4で割り、120 / 4 = 30分。"
        )
    },
    "q_07": {
        "title": "予定から逆算",
        "description": "10時15分到着、移動45分、準備25分のとき、準備開始時刻を求める。",
        "solution_guide": (
            "10時15分から移動45分を引くと9時30分出発。さらに準備25分を引き、9時5分開始。"
        )
    }
}
